...
func TestRedisPipeline(t *testing.T) {
	...
	pipeLine.Exec(c)
	returnedVal, has := c.Engine().Redis().Get(c, "test_set")
	assert.True(t, has)
	...
	pipeLine.Exec(c)
	_, has = c.Engine().Redis().Get(c, "test_set")
	assert.False(t, has)

	pipeLine = r.PipeLine()
	pipeLine.HSet(c, "test_hset", "a", "b", "c", "d")
	pipeLine.Exec(c)
	hSetValues := c.Engine().Redis().HGetAll(c, "test_hset")
	...
	pipeLine.Exec(c)
	returnedVal, hasVal := c.Engine().Redis().HGet(c, "test_inc", "a")
