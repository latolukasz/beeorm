...
func testSchema(t *testing.T, version int) {
	...
	alters = GetAlters(c)
	c.Engine().DB().Exec(c, alters[0].SQL)
	...
	c.Engine().DB().Exec(c, "ALTER TABLE `schemaEntity` DROP COLUMN `Name`")
	alters = GetAlters(c)
	assert.Len(t, alters, 1)
	assert.True(t, alters[0].Safe)
	if version == 5 {
	...
	c.Engine().DB().Exec(c, "ALTER TABLE `schemaEntity` CHANGE COLUMN `Year` `Year` varchar(255) NOT NULL DEFAULT ''")
	alters = GetAlters(c)
	assert.Len(t, alters, 1)
	if version == 5 {
	...
	c.Engine().DB().Exec(c, "ALTER TABLE `schemaEntity` ADD COLUMN `Year2` varchar(255) NOT NULL DEFAULT ''")
	alters = GetAlters(c)
	assert.Len(t, alters, 1)
	assert.Equal(t, "ALTER TABLE `test`.`schemaEntity`\n    DROP COLUMN `Year2`;", alters[0].SQL)
	c.Engine().DB().Exec(c, alters[0].SQL)

	c.Engine().DB().Exec(c, "ALTER TABLE `schemaEntity` DROP INDEX `TestIndex`")
	alters = GetAlters(c)
	assert.Len(t, alters, 1)
	assert.Equal(t, "ALTER TABLE `test`.`schemaEntity`\n    ADD INDEX `TestIndex` (`Name`,`Uint16`);", alters[0].SQL)
	c.Engine().DB().Exec(c, alters[0].SQL)
	...
	c.Engine().DB().Exec(c, "ALTER TABLE `schemaEntity` ADD INDEX `TestIndex2` (`Name`);")
	alters = GetAlters(c)
	assert.Len(t, alters, 1)
	...
	c.Engine().DB().Exec(c, "ALTER TABLE `schemaEntityRef` ADD COLUMN `Year2` varchar(255) NOT NULL DEFAULT ''")
	alters = GetAlters(c)
	assert.Len(t, alters, 1)
	...
	c.Engine().DB().Exec(c, "CREATE TABLE `TestDropTable` (`field` int(11) unsigned NOT NULL) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;")
	c.Engine().DB().Exec(c, "CREATE TABLE `invalid_table` (`field` int(11) unsigned NOT NULL) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;")
	alters = GetAlters(c)
	assert.Len(t, alters, 1)
