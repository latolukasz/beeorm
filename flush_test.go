...
func testFlush(t *testing.T, local bool, redis bool) {
	...
	assert.Equal(t, new(time.Time).UTC(), entity.TimeWithTime)
	assert.Equal(t, time.Date(2023, 8, 16, 12, 23, 11, 0, time.UTC), *entity.TimeNullable)
	assert.Equal(t, time.Date(2025, 11, 4, 21, 0, 5, 6, time.UTC), *entity.TimeWithTimeNullable)
	...
	assert.Equal(t, time.Date(1982, 11, 4, 21, 0, 5, 6, time.UTC), *entity.FlushStruct.TestTime)
