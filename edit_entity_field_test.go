...
func testUpdateFieldExecute(t *testing.T, async, local, redis bool) {
	...
	/* string */
	entity = GetByID[updateEntity](orm, ids[0])
	...
	entity = GetByID[updateEntity](orm, ids[1])
	...
	assert.NoError(t, err)
	entity = GetByUniqueIndex[updateEntity](orm, "Name", "name 100")
	assert.NotNil(t, entity)
	assert.Equal(t, ids[1], entity.ID)
	err = runEditEntityField(orm, entity, "Int", 100, async)
	assert.NoError(t, err)
	entity = GetByUniqueIndex[updateEntity](orm, "Multi", 13, 100)
	assert.NotNil(t, entity)
	assert.Equal(t, ids[1], entity.ID)
}
