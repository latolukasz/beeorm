...
func testLoadByIds(t *testing.T, local, redis bool) {
	...
	hasMissing := engine.LoadByIDs([]uint64{1, 2, 3, 4}, &rows, "*")
	...
	assert.NotNil(t, rows[2])
	assert.Nil(t, rows[3])
	hasMissing = engine.LoadByIDs([]uint64{1, 4, 4}, &rows)
	assert.True(t, hasMissing)
	assert.Len(t, rows, 3)
	assert.NotNil(t, rows[0])
	assert.Nil(t, rows[1])
	assert.Nil(t, rows[2])
	hasMissing = engine.LoadByIDs([]uint64{}, &rows)
	assert.False(t, hasMissing)
	assert.Len(t, rows, 0)
	engine.GetRedis().Del("a25e2:1")
	hasMissing = engine.LoadByIDs([]uint64{2, 4, 4, 1, 1, 4}, &rows)
	assert.True(t, hasMissing)
	assert.Len(t, rows, 6)
	assert.NotNil(t, rows[0])
	assert.Nil(t, rows[1])
	...
	hasMissing = engine.LoadByIDs([]uint64{1, 2, 3, 4}, &rows, "ReferenceOne/ReferenceTwo")
	assert.True(t, hasMissing)
	assert.Len(t, rows, 4)
	assert.Equal(t, "a", rows[0].Name)
	assert.Equal(t, "r1", rows[0].ReferenceOne.Name)
	...
	hasMissing = engine.LoadByIDs([]uint64{1}, &rows)
	assert.False(t, hasMissing)
	...
	rows = make([]*loadByIdsEntity, 0)
	hasMissing = engine.LoadByIDs([]uint64{1, 2, 3}, &rows)
	assert.False(t, hasMissing)
	assert.Len(t, rows, 3)
	assert.Equal(t, uint(1), rows[0].ID)
	assert.Equal(t, uint(2), rows[1].ID)
	...
	engine.GetRedis().FlushDB()
	hasMissing = engine.LoadByIDs([]uint64{2}, &rows)
	assert.False(t, hasMissing)
	rows = make([]*loadByIdsEntity, 0)
	hasMissing = engine.LoadByIDs([]uint64{1, 2, 3}, &rows)
	assert.False(t, hasMissing)
	...
	engine.GetRedis().FlushDB()
	hasMissing = engine.LoadByIDs([]uint64{3}, &rows)
	assert.False(t, hasMissing)
	rows = make([]*loadByIdsEntity, 0)
	hasMissing = engine.LoadByIDs([]uint64{1, 2, 3}, &rows)
	assert.False(t, hasMissing)
	...
	assert.False(t, hasMissing)
	rows = make([]*loadByIdsEntity, 0)
	hasMissing = engine.LoadByIDs([]uint64{1, 2, 3}, &rows, "ReferenceOne")
	assert.False(t, hasMissing)
	assert.Len(t, rows, 3)
	assert.Equal(t, uint(1), rows[0].ID)
	assert.Equal(t, uint(2), rows[1].ID)
	...
	hasMissing = engine.LoadByIDs([]uint64{3}, &rows, "ReferenceOne/ReferenceTwo")
	assert.False(t, hasMissing)
